...
-- 1.
CREATE VIEW v_productos_categoria_proveedor
AS
...
SELECT * FROM v_productos_categoria_proveedor;
 
 -- 2.
CREATE VIEW v_clientes_empleados AS
SELECT o.OrderID, o.OrderDate, c.CustomerName, e.LastName
FROM Orders o
...
SELECT p.ProductName, s.SupplierName, c.CategoryName, p.Price
FROM Products p
JOIN Suppliers s ON p.SupplierID = s.SupplierID
JOIN Categories c ON p.CategoryID = c.CategoryID
WHERE p.Price > 100;

SELECT * FROM v_proveedor_categoria;

-- 4.
CREATE VIEW v_clientes_ciudad AS
SELECT CustomerName, Address, PostalCode, Country
FROM Customers
WHERE City = 'London';

SELECT * FROM v_clientes_ciudad;

-- 5.
CREATE VIEW v_clientes_pedidos AS
...
-- 6.
CREATE VIEW v_empleadosnacidos AS
SELECT EmployeeID, FirstName, LastName, BirthDate
FROM Employees
WHERE BirthDate > '1990-01-01';

SELECT * FROM v_empleadosnacidos;

-- 7.
CREATE VIEW v_categoria_cantidad AS
SELECT c.CategoryName, p.ProductName, SUM(od.Quantity) AS Quantity
FROM Products p
JOIN Categories c ON p.CategoryID = c.CategoryID
JOIN OrderDetails od ON p.ProductID = od.ProductID
GROUP BY c.CategoryName, p.ProductName;

SELECT * FROM v_categoria_cantidad;

-- 8.
CREATE VIEW v_info_clientes_empleado AS
SELECT o.OrderID, o.OrderDate, c.CustomerName, c.Address, c.City, e.FirstName, e.LastName
FROM Orders o
JOIN Customers c ON o.CustomerID = c.CustomerID
JOIN Employees e ON o.EmployeeID = e.EmployeeID;

SELECT * FROM v_info_clientes_empleado;

-- 9.
CREATE VIEW v_proveedores_productos AS
SELECT s.SupplierName, s.ContactName, p.ProductName, p.Price
FROM Suppliers s
...
-- 10.
CREATE VIEW v_pedidos_fecha AS
...
SELECT * FROM v_pedidos_fecha;

-- 11.
CREATE VIEW v_productos_proveedor_inventario AS
SELECT s.SupplierName, COUNT(*) AS TotalProducts
FROM Products p
JOIN Suppliers s ON p.SupplierID = s.SupplierID
GROUP BY s.SupplierName;

SELECT * FROM v_productos_proveedores_inventario;

-- 12.
CREATE VIEW v_pedidos_cliente_fecha AS
SELECT o.OrderID, o.OrderDate, c.CustomerName, c.City
FROM Orders o
JOIN Customers c ON o.CustomerID = c.CustomerID
ORDER BY o.OrderDate;

SELECT * FROM v_pedidos_cliente_fecha;

-- 13.
CREATE VIEW v_empleados_fechanacimiento AS
SELECT EmployeeID, FirstName, LastName, BirthDate,
TIMESTAMPDIFF(YEAR, BirthDate, CURDATE()) AS Age
FROM Employees;

SELECT * FROM v_empleados_fechanacimiento;

-- 14.
CREATE VIEW v_busqueda_producto AS
SELECT p.ProductID, p.ProductName, s.SupplierName, c.CategoryName
FROM Products p
JOIN Suppliers s ON p.SupplierID = s.SupplierID
JOIN Categories c ON p.CategoryID = c.CategoryID
WHERE p.ProductName LIKE '%Anton%';

SELECT * FROM v_busqueda_producto;

-- 15.
CREATE VIEW v_info_ciudad_pais AS
SELECT CustomerName, City, Country
FROM Customers;

SELECT * FROM v_info_ciudad_pais;

-- 16.
CREATE VIEW v_cliente_empleado_fecha AS
SELECT o.OrderID, o.OrderDate, c.CustomerName, e.FirstName, e.LastName
FROM Orders o
JOIN Customers c ON o.CustomerID = c.CustomerID
JOIN Employees e ON o.EmployeeID = e.EmployeeID
...
SELECT * FROM v_cliente_empleado_fecha;

-- 17.
CREATE VIEW v_empleados_fotos AS
SELECT EmployeeID, CONCAT(FirstName, ' ', LastName) AS FullName, Photo
FROM Employees;

SELECT * FROM v_empleados_fotos;

-- 18.
CREATE VIEW v_productos_proveedor_categoria AS
...
SELECT * FROM v_productos_proveedor_categoria;

-- 19.
CREATE VIEW v_proveedores_pais_telefono AS
SELECT SupplierName, Country, Phone
FROM Suppliers;

SELECT * FROM v_proveedores_pais_telefono;

-- 20.
CREATE VIEW v_info_pedidos_cliente_fecha AS
SELECT o.OrderID, c.CustomerName, o.OrderDate, SUM(od.Quantity) AS TotalQuantity
FROM Orders o
JOIN Customers c ON o.CustomerID = c.CustomerID
JOIN OrderDetails od ON o.OrderID = od.OrderID
GROUP BY o.OrderID, c.CustomerName, o.OrderDate;

SELECT * FROM v_info_pedidos_cliente_fecha;
