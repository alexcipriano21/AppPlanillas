...
-- crear la base de datos Nortwind
CREATE DATABASE IF NOT EXISTS Northwind;
-- abrir la Base de datos de Nortwind
USE Northwind;
-- listar las bases de datos existentes
show databases;
-- si existen las tablas creadas que elimine
...
-- visualizar la estructura de la tabla Customers
 desc Customers;
 -- visualizar los campos con las llaves primarias
 show tables;
...
-- CLAUSULA WHERE
-- listar los productos cuyo precio sea mayor a 15
select ProductID, ProductName, price from products where Price>15;
-- listar los productos que comprenda entre 15 y 50
-- operadores (OR, AND, NOT)
select ProductID, ProductName, price from products where Price>=15 and price<=50 order by 3;
select ProductID, ProductName, price from products where Price>=15 OR price<=50 order by 3;
select ProductID, ProductName, price from products where NOT Price>=5 and price<=10 order by 3;
-- BETWEEN
select ProductID, ProductName, price from products where Price between 15 and 50 order by 3;
select ProductID, ProductName, price from products where NOT Price between 15 and 50 order by 3;
-- ORDENAR COLUMNA CLAUSULA ORDER BY
select * from customers order by Country;
select * from customers order by 7;
select CustomerId, customerName, City from customers order by City;

-- operador like
-- _ cualquier caracter
-- % cualquier cadena de cero o más caracteres
-- []  cualquier caracter individual del intervalo([a-z])
select * from employees;
-- LISTAR LOS EMPLEADOS CUYO NOMBRE COMIENCEN CON LA LETRA D
select employeeId, lastName from employees where LastName LIKE 'D%';
-- LISTAR LOS EMPLEADOS CUYO NOMBRE TERMINEN CON LA LETRA N
select employeeId, lastName from employees where LastName LIKE '%N';
-- listar los empleados en cuya nota diga la palabra Luara
select employeeId, lastName, Notes from employees where Notes LIKE '%Laura%';
-- LISTAR LOS EMPLEADOS CUYO NOMBRE QUE NO COMIENCEN CON LA LETRA D
select employeeId, lastName from employees where LastName not LIKE 'D%';
-- listar los empleados en cuya nota NO diga la palabra Luara
select employeeId, lastName, Notes from employees where Notes not LIKE '%Laura%';
-- Listar la tabla orderdetails cuyo segundo numero comiencen con la letra 0 del campo orderID
select * from orderdetails where orderId like '_0248';
-- seleccionar todas las ordenes  de la tabla orderdetails donde el dato almacenado
-- en el campo orderId comience con 10 en el tercer digito contenga un numero entre 1 y 5
-- y termine  con los digitos 48
 select * from orderdetails where orderId like '10[1-5]48';
 SELECT * FROM orderdetails WHERE orderId REGEXP '^10[1-5]48$';
 -- order by
 select * from products order by SupplierID;
  select * from products order by 3;
   select * from products order by SupplierID asc;
   select * from products order by SupplierID desc;
     select * from products order by 3 desc;
     -- OPERADOR DISTINCT ELIMINA DATOS DUPLICADOS
     select * from products;
...
LIMIT 3,10;  
-- ASIGNAR NOMBRES AS
SELECT productId AS 'NOMBRE DEL PRODUCTO' from products;
SELECT * FROM products;
SELECT productiD AS 'CODIGO', productName AS 'NOmbre del productos', Price as 'Precio'
from products;
-- clausula UPODATE, MODIFICAR O REEMPLAZAR
SELECT * FROM products;
update products set PRICE=100 where supplierId=1;
update products set PRICE=200 where productID=1;
update products set PRICE=300 where productID=2;
SELECT * FROM customers;
-- MDIFICAR LA DIRECCION DE LOS CLIENTES, CON LA AV. MEXICO 144, AQUELLOS CLIENTES QUE SON
-- DEL PAIS FRANCE Y SPAIN
SELECT * FROM customers WHERE Country IN ('France','Spain') order by Country;
update customers set Address='Av. mexico 144' WHERE Country IN ('France','Spain');
-- Modificar la dirección de los clientes con la Av. perrú 125, de aquellos clientes
-- que seon de la ciudad de barcelo y Madrid.
Update customers set city='barceloana' Where city in('Av. Perú 125');
select * from customers where Country='Spain';
update customers set city='Madrid' where customerId in(8,22);
...
create procedure ejemplo1(
  in pais char (50))
  select * from Customers where country=pais ORDER BY country;
  end$$
  delimiter ;
  
call ejemplo1('Germany');

drop procedure ejemplo020;
delimiter $$
create procedure ejemplo020(
  in p1 char(50),
  in p2 char(50))
  select * from Customers where country in(p1, p2) ORDER BY country;
  end$$
  delimiter ;

call ejemplo020('Germany','Brazil');
call ejemplo020('Mexico','Argentina');

SELECT * FROM orderdetails;
SELECT * FROM orders;

delimiter //
create procedure usp_obtenerdetallecliente(IN customerID INT)
BEGIN
...
call usp_obtenerdetallecliente(2);
call usp_obtenerdetallecliente(3);

SELECT * FROM products;
SELECT * FROM categories;

delimiter //
CREATE PROCEDURE usb_productoscategoria(IN categoryID INT)
BEGIN
...
call usp_productoscategoria(1);
call usp_productoscategoria(2);

SELECT * FROM orders;
SELECT * FROM shippers;

drop procedure usp_obtenerpedidostransportista;

delimiter // 
CREATE PROCEDURE usp_obtenerpedidostransportista(IN shipperID INT)
BEGIN 
    SELECT o.*
    FROM Orders o
...
call usp_obtenerpedidostransportista(1);
call usb_obtenerpedidostransportista(2);

SELECT * FROM products;
SELECT * FROM suppliers;

drop procedure usp_productosproveedor;

delimiter //
CREATE PROCEDURE usp_productosproveedor(IN supplierID INT)
BEGIN
    SELECT p.*
    FROM Products p
...
call usp_productosproveedor(1);
call usp_productosproveedor(2);

delimiter //
CREATE PROCEDURE usp_categoria( IN searchTerm VARCHAR(50), IN categoryID INT)
BEGIN
    SELECT p.*
    FROM Products p
    INNER JOIN Categories c ON p.CategoryID = c.CategoryID
    WHERE c.CategoryID=categoryID
    AND p.ProductName LIKE CONCAT('%', searchTerm, '%');
END //
delimiter ;

call usp_categoria('h', 2);
call usp_categoria('c', 3);

delimiter //
CREATE PROCEDURE usp_buscarnombreciudad(IN searchTerm VARCHAR(50), IN city VARCHAR(50))
BEGIN
    SELECT *
    FROM Customers
    WHERE CustomerName LIKE CONCAT('%', searchTerm, '%')
    AND City = city;
END //
delimiter ;

call usp_buscarnombreciudad('Alfreds', 'Berlin');
call usp_buscarnombreciudad('Bon app', 'Marseille');

delimiter //
CREATE PROCEDURE usp_pedidosporrangosfechas(IN employeeID INT, IN startDate DATE, IN endDate DATE)
BEGIN
    SELECT o.*
    FROM Orders o
...
call usp_pedidosporrangosfechas(1, '1996-08-01', '1996-08-07');
call usp_pedidosporrangosfechas(1, '1996-09-12', '1996-09-20');

delimiter //
CREATE PROCEDURE usp_obtenerdetallespedido(IN customerID INT, IN searchTerm VARCHAR(50))
BEGIN
    SELECT o.Customerid, p.ProductName, od.*
    FROM OrderDetails od
    INNER JOIN Orders o ON od.OrderID = o.OrderID
...
call usp_obtenerdetallespedido(63, 'Cha');
call usp_obtenerdetallespedido(20, 'Che');

drop procedure usp_obtenerproductossuminstrados;

delimiter //
CREATE PROCEDURE usp_obtenerproductossuminstrados(IN supplierID INT, IN minPrice DECIMAL(10,2), IN maxPrice DECIMAL(10,2))
BEGIN
    SELECT p.*
    FROM Products p
    INNER JOIN Suppliers s ON p.SupplierID = s.SupplierID
...
call usp_obtenerproductossuminstrados(1, 18, 20);
call usp_obtenerproductossuminstrados(4, 10, 31);

delimiter //
CREATE PROCEDURE usp_pedidosclientes(IN countryList VARCHAR(200))
BEGIN
    SELECT c.country, o.*
    FROM Orders o
...
call usp_pedidosclientes('Mexico');
call usp_pedidosclientes('UK');

delimiter //
CREATE PROCEDURE usp_productoscategoria(IN categoryID INT, IN supplierList VARCHAR(200))
BEGIN
    SELECT p.*
    FROM Products p
    INNER JOIN Categories c ON p.CategoryID = c.CategoryID
    INNER JOIN Suppliers s ON p.SupplierID = s.SupplierID
    WHERE c.CategoryID = categoryID
    AND FIND_IN_SET(s.SupplierID, supplierList);
END //
delimiter ;

call usp_productoscategoria(1, '1,10,16,18');
call usp_productoscategoria(2, '1,2,3,6,7');

delimiter //
CREATE PROCEDURE usp_clientesciudad(IN city VARCHAR(50), IN shipperList VARCHAR(200))
BEGIN
    SELECT c.city, o.*
    FROM Orders o
    INNER JOIN Customers c ON o.CustomerID = c.CustomerID
    INNER JOIN Shippers s ON o.ShipperID = s.ShipperID
    WHERE c.City = city
    AND FIND_IN_SET(s.ShipperID, shipperList);
END //
delimiter ;

call usp_clientesciudad('Madrid', '1,3');
call usp_clientesciudad('Graz', '1,2,3');

delimiter //
CREATE PROCEDURE usp_clientespedidos(IN startDate DATE, IN endDate DATE)
BEGIN
    SELECT *
    FROM Customers
    WHERE CustomerID IN (
...
call usp_clientespedidos('1996-09-10', '1996-09-16');
call usp_clientespedidos('1997-01-01', '1997-01-07');

delimiter //
CREATE PROCEDURE usp_busqueda_apellido(IN lastNamePrefix VARCHAR(50))
BEGIN
    SELECT *
    FROM Employees
    WHERE LastName LIKE CONCAT(lastNamePrefix, '%');
END //
delimiter ;

call usp_busqueda_apellido('D'); 
call usp_busqueda_apellido('C'); 
call usp_busqueda_apellido('F');

delimiter //
CREATE PROCEDURE usp_proveedorespaises(IN countryList VARCHAR(200))
BEGIN
    SELECT s.Country, p.*
    FROM Products p
    INNER JOIN Suppliers s ON p.SupplierID = s.SupplierID
    WHERE s.Country IN (countryList);
END //
delimiter ;

call usp_proveedorespaises('UK');
call usp_proveedorespaises('USA');

delimiter //
CREATE PROCEDURE usp_clientesrangospedidos(IN minQuantity INT, IN maxQuantity INT)
BEGIN
    SELECT c.*, od.Quantity
    FROM Customers c
    INNER JOIN Orders o ON c.CustomerID = o.CustomerID
    INNER JOIN OrderDetails od ON o.OrderID = od.OrderID
    WHERE od.Quantity BETWEEN minQuantity AND maxQuantity
    ORDER BY od.Quantity;
END //
delimiter ;

call usp_clientesrangospedidos(10, 30);
call usp_clientesrangospedidos(14, 16);

delimiter //
CREATE PROCEDURE usp_rangofechaclientes(IN startDate DATE, IN endDate DATE)
BEGIN
...
call usp_rangofechaclientes('1997-01-21', '1997-01-23');
call usp_rangofechaclientes('1996-07-10', '1996-07-18');

delimiter //
CREATE PROCEDURE usp_productosnombresprecio(IN searchTerm VARCHAR(50), IN price1 INT, IN price2 INT)
BEGIN
    SELECT *
    FROM Products
...
call usp_productosnombresprecio('C', 18, 19);
call usp_productosnombresprecio('si', 10, 81);

-- Crear 10 procedimientos almacenados con sus propios ejemplos:
-- 1.-
DELIMITER //
CREATE PROCEDURE usp_buscarClientesNombreCiudad(
    IN nombreCliente VARCHAR(50),
    IN ciudadCliente VARCHAR(50)
)
BEGIN
    SELECT *
    FROM Customers
    WHERE CustomerName LIKE CONCAT('%', nombreCliente, '%')
    AND City LIKE CONCAT('%', ciudadCliente, '%');
END //
DELIMITER ;

CALL usp_buscarClientesNombreCiudad('%', '%')

-- 2.-
DELIMITER //
CREATE PROCEDURE usp_categoriasPorDescripcion(
    IN descripcionCategoria VARCHAR(255)
)
BEGIN
    SELECT *
    FROM Categories
    WHERE Description LIKE CONCAT('%', descripcionCategoria, '%');
END //
DELIMITER ;
CALL usp_categoriasPorDescripcion('');

-- 3.-
DELIMITER //
CREATE PROCEDURE usp_productosPorCantidadVendida(
    IN cantidadMin INT,
    IN cantidadMax INT
)
BEGIN
    SELECT *
    FROM OrderDetails
    WHERE Quantity BETWEEN cantidadMin AND cantidadMax;
END //
DELIMITER ;
CALL usp_productosPorCantidadVendida(1, 50);

drop procedure usp_proveedoresPorCiudadNombre;
-- 4.-
DELIMITER //
CREATE PROCEDURE usp_proveedoresPorCiudadNombre(
    IN ciudadProveedor VARCHAR(50),
    IN nombreProveedor VARCHAR(50)
)
BEGIN
    SELECT *
    FROM Suppliers
    WHERE City LIKE CONCAT('%', ciudadProveedor, '%')
    AND SupplierName LIKE CONCAT('%', nombreProveedor, '%');
END //
DELIMITER ;
CALL usp_proveedoresPorCiudadNombre('Tokyo', 'Tokyo Traders');

-- 5.-
DELIMITER //
CREATE PROCEDURE usp_categoriasPorNombreDescripcion(
...
BEGIN
    SELECT *
    FROM Categories
    WHERE CategoryName LIKE CONCAT('%', nombreCategoria, '%')
    OR Description LIKE CONCAT('%', descripcionCategoria, '%');
END //
DELIMITER ;
CALL usp_categoriasPorNombreDescripcion('Beverages', 'Soft drinks');

-- 6.-
DELIMITER //
CREATE PROCEDURE usp_transportistasPorNombreRegion(
    IN nombreTransportista VARCHAR(50),
    IN regionTransportista VARCHAR(50)
)
BEGIN
    SELECT *
    FROM Shippers
    WHERE CompanyName LIKE CONCAT('%', nombreTransportista, '%')
    AND Region LIKE CONCAT('%', regionTransportista, '%');
END //
DELIMITER ;
CALL usp_transportistasPorNombreRegion('Speedy', 'WA');
